#这个代码运行后，我点击“展开”没效果，为什么，如何修改。是因为本来就是展开的所以不会再展开吗？
...
for index, row in df.iterrows():
    ...
    # 如果该规则对应的 Frame 已经存在，则将新的 Frame 添加到已有的列表中
    if pattern in rule_frames:
        rule_frames[pattern].append(tk.Frame(output_frame, relief=tk.RIDGE, bd=2))
    else:
        # 否则创建一个新的列表存储该规则对应的 Frame
        rule_frames[pattern] = [tk.Frame(output_frame, relief=tk.RIDGE, bd=2)]
