        ...
        for line_num, line in enumerate(f, start=1):
            for index, rule in enumerate(rules):
                if match_rule(rule, line):
                    ...
                    output_dict[pattern]['匹配项'].append(f"Line {line_num}: {line.strip()}")
                    break

    for pattern, value in output_dict.items():
        count = len(value['匹配项'])
        result = df.loc[df['规则']==pattern, '结果'].values[0]
        code = df.loc[df['规则']==pattern, '代码'].values[0]
        output_dict[pattern]['结果'] = f"{result}，匹配次数：{count}"
        output_dict[pattern]['代码'] = code
...
with open(filename, 'r', encoding='utf-8') as f:
    ...
    for pattern, value in output_dict.items():
        output_text.insert('end', f"规则：{pattern}\n")
        output_text.insert('end', f"结果：{value['结果']}\n")
        output_text.insert('end', f"代码：{value['代码']}\n")
        output_text.insert('end', "匹配项：\n")
        for item in value['匹配项']:
